...
| Client definition (v3) | Established small businesses and funded founders with a run-rate problem. Values filter unchanged (intent, not industry). |
...
| Core proposition (v3) | "You don't need a bigger team. You need a better system." Old hero line retired from the hero; lineage preserved in About section copy. |
| Positioning | Presence partner (build + run a client's brand/site/content on retainer) — recommended over the "AI consultancy" reading of the 25 June stack pivot. The delivery pipeline is the mechanism, not the pitch. |
| Offer architecture | Launch (build engagement) / Run (ongoing retainer, centre of gravity). Campaign folded into Run rather than named as a third tier. |
| Retainer band | New Run proposals anchor at £1,500–£3,000/mo — raised from the plan's original £1,200–£2,500 draft on the view that the delivery system is now proven, not speculative. FSC's existing £400/wk (~£1,733/mo) stays as-is; not renegotiated as part of this decision. Site copy stays qualitative; band used in proposals only. |
| PureMed on the MSS site | Off-site lock reversed: once PureMed is live, it becomes the flagship proof of the new offer on the MSS site. Requires client comfort — to be asked before it's shown. |
...
| Bricks skill system | Added 19 June 2026. The original single `bricks-html-importer` skill was being violated in production (Code elements used for content where native elements should have been used) and mixed three separate concerns. Replaced with four skills: `bricks-site-connection` (MCP and environment setup), `bricks-lowcode-skill` (element and style reference catalogue), `bricks-html-importer` (HTML-to-Bricks conversion workflow), and `bricks-mss-site-notes` (MSS-specific build history and fixes). See Flagged for a verification gap in the element catalogue. **Update 3 July 2026:** Bricks retired for new builds (Astro pivot, 25 June). The four skills stay installed for legacy live-site fixes only — MSS production site until its Astro cutover — then can be archived. |
| Higgsfield asset generation skill | Added 22 June 2026. Generates a full cross-section of image and video assets for a client site in a single job. Reads the HTML prototype and brand documents, maps every placement, writes generation briefs grounded in brand and constraints, selects models, runs all jobs via the Higgsfield MCP, and produces a placement manifest in the client folder. Lives in ~/.claude/skills/higgsfield-asset-generation/ alongside the other global skills. Requires Higgsfield MCP connected at https://mcp.higgsfield.ai/mcp — add to global Claude Code MCP config before first use. Plus plan free image models (Flux.2 Pro, GPT Image, Seedream 4.5, Kling O1 Image, Nano Banana, Seedream 5.0 Lite) must have the free toggle enabled in Higgsfield. Video generation draws credits — reserved for hero placements only. |
...
| Domain | `mainstagestudio.co.uk`. Live and published as of mid-June 2026, hosted on Cloudways via WordPress and Bricks. Homepage only published so far. The live site remains WordPress + Bricks until its Astro cutover completes — `mss-production-ops.md` governs any live-site change until then. ~~A previous version of this log incorrectly recorded the domain as `mainstagingstudio.co.uk` — confirmed and corrected 20 June 2026.~~ |
...
| Bricks layout element for layout wrappers | **Container**, not Div. Confirmed 21 June 2026: live MSS homepage has 33 Container elements and 2 Div elements. The 2 Divs are the empty atmospheric-image overlay elements in dark sections — they stay as Div (they're purely positioned, no flex/direction controls needed). All other layout wrappers inside sections are Containers. The old Div-only rule in `mss-bricks-build-guide.md`'s checklist has been corrected. All four skills (`bricks-lowcode-skill` Rule 1, `bricks-html-importer`, `bricks-mss-site-notes`) consistently say Container. |
